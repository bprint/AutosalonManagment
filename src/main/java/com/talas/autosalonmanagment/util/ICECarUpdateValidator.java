...
@Component
public class ICECarUpdateValidator implements Validator {
    private final ICECarsServiceImpl icecarsService;

    @Autowired
    public ICECarUpdateValidator(ICECarsServiceImpl icecarsService) {
    ...
    public void validate(Object target, Errors errors) {
        ...
        if (!icecarsService.findByVin(icecar.getVin()).isPresent())
            errors.rejectValue("vin", "A car with the same VIN is not exist");
    }
}
